alter table public.products add column if not exists hsn_sac text;
alter table public.inventory add column if not exists minimum_stock_level numeric default 0 check (minimum_stock_level >= 0);
...
begin
  ...
  insert into public.inventory (
    item_name, quantity, quantity_cached, unit, low_stock_threshold, minimum_stock_level, purchase_rate, selling_price, location, product_id
  ) values (
