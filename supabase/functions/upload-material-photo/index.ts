...
Deno.serve(async (req: Request) => {
  ...
  if (!jobId || !materialName || !imageFile) {
    return new Response(JSON.stringify({ error: 'Missing required fields: jobId, materialName, image' }), {
      status: 400, headers: { ...CORS, 'Content-Type': 'application/json' },
    });
  }

  // Lookup jobCode
  const { data: jobData, error: jobError } = await supabaseUser.from('jobs').select('job_code').eq('id', jobId).single();
  const jobCode = jobData?.job_code || jobId;

  const filename = `${sanitizeFilenameSegment(jobCode)}-${sanitizeFilenameSegment(materialName)}-${Date.now()}.webp`;
