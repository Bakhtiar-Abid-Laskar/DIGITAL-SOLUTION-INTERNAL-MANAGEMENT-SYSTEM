...
export function AppConfigProvider({ children }: { children: React.ReactNode }) {
  ...
  const getRoleColor = React.useCallback((roleId: string) => {
    return config.roles.find(r => r.id === roleId)?.color_hex || '#9CA3AF';
  }, [config.roles]);
  ...
  const value = React.useMemo(() => ({
    ...
    getRoleColor
  }), [config, loading, error, refreshConfig, getJobStatusColor, getPriorityColor, getServiceLocationColor, getRoleColor]);
